...
VENV_ACTIVATE = VENV_DIR / ("Scripts/activate" if OS_NAME == "Windows" else "bin/activate")
...
def install_package(package_name):
    try:
        subprocess.run([str(VENV_DIR / "bin/pip" if OS_NAME != "Windows" else VENV_DIR / "Scripts/pip"),
...
PYTHONPATH_ENTRY = f'export PYTHONPATH="{SCRIPT_DIR}:$PYTHONPATH"' if OS_NAME != "Windows" else f'set PYTHONPATH={SCRIPT_DIR};%PYTHONPATH%'
...
if OS_NAME != "Windows":
    ...
    activate_command = f'cmd /c "{VENV_ACTIVATE}.bat && echo Activated virtual environment"'
